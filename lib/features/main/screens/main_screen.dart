...
class MainScreen extends BaseScreen<MainController> {
  ...
  Widget buildScreen(
    ...
    return BaseScaffold(
      bottomNavigationBar: Obx(() => _buildBottomNavigationBar(isDarkMode)),
      body: Obx(() => _buildBody()),
    );
  }

  NavigationBar _buildBottomNavigationBar(bool isDarkMode) {
  ...
  Widget _buildBody() {
    return IndexedStack(
      index: vm.currentPageIndex.value,
      children: [
        HomeScreen(),
        StoreScreen(),
        GetBuilder<WishlistController>(
          init: WishlistController(), // Khởi tạo controller ở đây
